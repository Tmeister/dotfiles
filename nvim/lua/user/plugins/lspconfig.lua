...
vim.diagnostic.config {
    ...
    float = {
        ...
        format = function(diagnostic)
            if diagnostic.user_data ~= nil and diagnostic.user_data.lsp.code ~=
                nil then
                return string.format("%s: %s", diagnostic.user_data.lsp.code,
                                     diagnostic.message)
            end
    ...
    settings = {
        intelephense = {
            stubs = {
                ...
                "sysvsem", "sysvshm", "tokenizer", "xml", "xdebug", "xmlreader",
                "xmlwriter", "yaml", "zip", "zlib", "wordpress", "woocommerce",
                "acf-pro", "wordpress-globals", "wp-cli", "polylang"
            },
            environment = {
                includePaths = '/home/tmeister/.composer/vendor/php-stubs/'
            },
            files = {maxSize = 5000000}
        }
    },
    root_dir = require'lspconfig'.util.root_pattern("wp-config.php",
                                                    "composer.json", ".git")
}
