...
echo -e "${GREEN}✓ GNU Stow $(stow --version | head -1 | awk '{print $NF}')${NC}"
echo ""
...
for pkg in "${PACKAGES[@]}"; do
    ...
    if [ -d "$DOTFILES_DIR/$name" ]; then
        echo -e "  ${GREEN}✓${NC} ${CYAN}$name${NC} - $desc"
    else
        echo -e "  ${RED}✗${NC} ${CYAN}$name${NC} - ${RED}directory not found${NC}"
    fi
done
...
if [ ${#CONFLICTS[@]} -gt 0 ]; then
    ...
    for conflict in "${CONFLICTS[@]}"; do
        ...
        echo -e "  ${YELLOW}⚠${NC} $file (from package: ${CYAN}$pkg${NC})"
    done
...
for dir in "${REQUIRED_DIRS[@]}"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
        echo -e "  ${GREEN}✓${NC} Created: ${dir/#$HOME/~}"
    else
        echo -e "  ${CYAN}→${NC} Exists: ${dir/#$HOME/~}"
    fi
done
...
for pkg in "${PACKAGES[@]}"; do
    ...
    if stow --verbose=1 "$name" 2>&1 | grep -q "ERROR"; then
        echo -e "  ${RED}✗${NC} Failed: ${CYAN}$name${NC}"
        STOW_FAILED+=("$name")
    else
        stow --restow "$name" &>/dev/null
        echo -e "  ${GREEN}✓${NC} Stowed: ${CYAN}$name${NC}"
    fi
done
...
if [ -f "$HOME/.config/omarchy/hooks/post-update" ]; then
    ...
    echo -e "  ${GREEN}✓${NC} Omarchy post-update hook installed"
else
    echo -e "  ${YELLOW}⚠${NC} Omarchy post-update hook not found"
fi

# Seed omarchy configuration if omarchy is installed
if [ -d "$HOME/.local/share/omarchy" ]; then
    echo -e "  ${CYAN}→${NC} Seeding Hyprland configuration with overrides..."
    if command -v omarchy-seed-config &>/dev/null; then
        if omarchy-seed-config &>/dev/null; then
            echo -e "  ${GREEN}✓${NC} Hyprland config seeded successfully"
        else
            echo -e "  ${YELLOW}⚠${NC} Hyprland config already seeded"
        fi
    else
        echo -e "  ${YELLOW}⚠${NC} omarchy-seed-config not yet in PATH (run 'exec zsh' then seed manually)"
    fi
else
    echo -e "  ${CYAN}→${NC} Omarchy not detected, skipping hypr config seeding"
fi
